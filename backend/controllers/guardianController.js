...
exports.getAllGuardians = async (req, res) => {
  ...
  try {
    const guardians = await Guardian.find({ branchId })
      .populate("userId", "username email userRole")
      .populate("branchId", "branchName branchType");
    sendSuccessResponse(
      res,
      200,
...
exports.updateGuardian = async (req, res) => {
  ...
  try {
    ...
    if (password) {
      ...
      const userId = updatedGuardian.userId;
      if (!userId) {
        return sendErrorResponse(
          res,
          400,
          "Associated user not found for this guardian"
        );
      }
...
exports.getGuardianByCnic = async (req, res) => {
  ...
  try {
    const guardian = await Guardian.findOne({ cnicNumber: cnic }).populate(
      "userId"
    );
    if (!guardian) {
      return sendErrorResponse(res, 404, "Guardian not found");
